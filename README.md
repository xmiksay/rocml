...
| `ornith-1.5-35b` | qwen3.5-moe | `Ornith-1.5-35B-A3B-GGUF/Ornith-1.5-35B-Q4_K_M.gguf` | temp 0.6, top_p 0.95, top_k 20 | on |
...
**Usage reporting.** Every response's `usage.prompt_tokens_details.cached_tokens` reports how many of that request's prompt tokens were restored from a conversation-prefix snapshot instead of being re-prefilled (`rocml::snapshot::turn::TurnOutcome::reused_prefix`) — `0`, not omitted, on a snapshot miss or when the snapshot layer is off. A streaming request can additionally set `stream_options: {"include_usage": true}` to get OpenAI's usual streaming-usage shape: every chunk carries `"usage": null` except one extra chunk sent after the `finish_reason` chunk (and before `[DONE]`) with empty `choices` and the full `usage` object.

**Thinking-mode snapshot reuse (issue #12).** A thinking-enabled model's own generation prompt (`<think>\n`) and its history rendering of a past turn's *stripped* reasoning (`<think>\n\n</think>\n\n`) are two different strings at the position `run_turn`'s pre-existing prefill-boundary/end-of-turn snapshots cut on, so neither snapshot's token sequence is reproduced by the next turn's re-render — every thinking-enabled multi-turn request paid a full re-prefill. `rocml-serve`/`rocml-cli chat` now also compute a render-stable cut via `rocml::chat::render_with_boundary` (right after the last history message's `<|im_end|>` special token, which can never BPE-merge with adjacent text) and pass it to `run_turn` as `stable_boundary`, forcing an extra mid-prefill capture there. That snapshot's prefix — "the whole conversation up through the previous turn's last message" — is exactly what every later turn's history re-render reproduces byte-for-byte, snapshot miss or not.

**GPU-resident snapshot tier.** In a single growing conversation the state the next turn needs is still sitting in VRAM when the request arrives, so `rocml-serve`/`rocml-cli chat`/`bench --turns` no longer round-trip it through host RAM: the model keeps two on-device rewind points (the render-stable boundary above, and the end of turn) holding only what later tokens can overwrite — GDN recurrence state plus, under `--kv-cache q8`/`q4-mixed`, each mixed layer's fp16 recent window (~53 MiB each on Ornith-1.0-9B, reserved in the VRAM budget) — and rewinds to the longest one whose token prefix matches, in microseconds. The RAM/disk store (`--snapshot-ram-mb`/`--snapshot-dir`) is now the *fallback* for a second conversation, an edited history, or (disk only) a restarted server, and it keeps one *pinned* snapshot per turn — the one the next turn is expected to hit — that eviction can't displace; previously, at ~78K tokens under `q4-mixed`, a turn's own later captures evicted it and every turn thereafter re-prefilled from scratch. The per-request log line names the tier: `snapshot hit source=gpu|ram|disk`. `--snapshot-ram-mb 0` with no `--snapshot-dir` still turns the whole layer off, GPU tier included.
