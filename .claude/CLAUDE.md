...
- `rocml-serve`'s `server_e2e`: spawns the real router on an ephemeral port (Qwen3.5-2B) and drives it with a hand-rolled minimal HTTP/1.1 client (`tests/support/`) — non-streamed completion, streamed SSE, and a tools request. A companion `#[ignore]`d test in the same file (`ornith_tool_call_is_emitted`) asserts a real tool call against the Ornith GGUF; run it explicitly (`cargo test --release -p rocml-serve --test server_e2e -- --ignored`) rather than alongside the default test, since the two would compete for VRAM. Both use a `JoinHandle`-based shutdown sequence (see `worker::spawn`'s doc comment) — skipping it crashes the test process on exit with "pure virtual method called" from a still-live worker thread racing the HIP driver's own teardown.
