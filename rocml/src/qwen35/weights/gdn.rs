...
use crate::weights::{load_matrix_f16, load_matrix_f32, load_vector_f32};

pub struct GdnLayerWeights {
    pub attn_norm: DeviceBuffer<f32>,
    /// gemv_f16 shape (m=conv_dim, n=hidden): fused `[Q|K|V]` projection.
    pub attn_qkv: DeviceBuffer<f16>,
    /// gemv_f16 shape (m=value_dim, n=hidden): the output silu-gate `Z`.
    pub attn_gate: DeviceBuffer<f16>,
    /// gemv_f16 shape (m=num_v_heads, n=hidden): write-strength logit `B`.
    pub ssm_beta: DeviceBuffer<f16>,
    /// gemv_f16 shape (m=num_v_heads, n=hidden): decay logit `A`.
    pub ssm_alpha: DeviceBuffer<f16>,
    ...
    /// gemv_f16 shape (m=hidden, n=value_dim).
    pub ssm_out: DeviceBuffer<f16>,
...
impl GdnLayerWeights {
    pub(crate) fn load(
        ...
        Ok(Self {
            ...
            attn_qkv: load_matrix_f16(gguf, &format!("{p}.attn_qkv.weight"), gdn.conv_dim, hidden)?,
            attn_gate: load_matrix_f16(
                gguf,
                &format!("{p}.attn_gate.weight"),
                gdn.value_dim,
                hidden,
            )?,
            ssm_beta: load_matrix_f16(
                gguf,
                &format!("{p}.ssm_beta.weight"),
                gdn.num_v_heads,
                hidden,
            )?,
            ssm_alpha: load_matrix_f16(
                gguf,
            ...
            ssm_out: load_matrix_f16(gguf, &format!("{p}.ssm_out.weight"), hidden, gdn.value_dim)?,
