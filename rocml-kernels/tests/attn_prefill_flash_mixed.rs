//! GPU integration test for the chunked-prefill mixed-KV attention kernels
//! (`attn_prefill_flash_partial_mixed_q8`/`_q4` +
//! `attn_prefill_flash_reduce_f32`, `kernels/attn_prefill_flash_mixed.hip`)
//! against a CPU causal-softmax-attention reference — the chunked-prefill
//! sibling of `attn_decode_mixed.rs`'s decode-attention test, extended to
//! several query rows (a whole chunk) instead of one.
//!
//! `sink_len`/`window_len` here are small test-local values, independent of
//! `rocml::kv_quant::layout`'s production `SINK_LEN`/`WINDOW_LEN` constants
//! — only the Rust-side cache orchestration (`MixedAttnPlane`) uses those.
//! As with the production path, the cache is assumed already fully
//! populated through this chunk's own newest positions by the time
//! attention runs (append-before-attend) — this test exercises the *read*
//! kernel only, not `MixedAttnPlane::append_chunk`'s own bit-identity gate
//! (covered by `mixed_kv_chunked_prefill_parity.rs` at the model level).
use half::f16;
use rocml_hip::{kernel_params, Device, DeviceBuffer, LaunchConfig, Module};

const TOL: f32 = 1.5e-2; // quantization + f16 rounding, matches attn_decode_mixed.rs's bound
/// Must match `BR`/`BC` in `kernels/attn_prefill_flash_mixed.hip`.
const BR: u32 = 8;
const BC: u32 = 16;
...
fn quantize_k(block: &[f32], window_len: usize, head_dim: usize) -> (Vec<i8>, Vec<f32>) {
    let mut codes = vec![0i8; block.len()];
    let mut scales = vec![0f32; head_dim];
    for d in 0..head_dim {
        let mut max_abs = 0f32;
        for t in 0..window_len {
            max_abs = max_abs.max(block[t * head_dim + d].abs());
        }
...
fn quantize_v_q8(block: &[f32], window_len: usize, head_dim: usize) -> (Vec<i8>, Vec<f32>) {
    let mut codes = vec![0i8; block.len()];
    let mut scales = vec![0f32; window_len];
    for t in 0..window_len {
        let row = &block[t * head_dim..(t + 1) * head_dim];
...
fn quantize_v_q4(block: &[f32], window_len: usize, head_dim: usize) -> (Vec<u8>, Vec<f32>) {
    let mut packed = vec![0u8; window_len * (head_dim / 2)];
    let mut scales = vec![0f32; window_len];
    for t in 0..window_len {
        let row = &block[t * head_dim..(t + 1) * head_dim];
        let max_abs = row.iter().fold(0f32, |m, &v| m.max(v.abs()));
        ...
        for pair in 0..head_dim / 2 {
            ...
            packed[t * (head_dim / 2) + pair] = ((lo as u8) & 0x0F) | (((hi as u8) & 0x0F) << 4);
        }
    }
    (packed, scales)
}

/// Causal softmax attention over `[0, cur_len)`, given the *already
/// resolved* per-position K/V rows (single kv head, `group` q heads sharing
/// it — mirrors `attn_decode_mixed.rs`'s reference builder).
fn cpu_reference(
    q_row: &[f32],
    k_by_pos: &[Vec<f32>],
    v_by_pos: &[Vec<f32>],
...
struct Scenario {
    ...
    sink_len: u32,
    window_len: u32,
    /// How many positions past `window_base` are currently valid (populated
    /// by this chunk's own append, per the append-before-attend contract).
    window_fill: u32,
    chunk_len: u32,
    n_splits: u32,
}
...
fn run_mixed_flash(scenario: Scenario, v_bits: u32) {
    let Scenario {
        n_heads,
        head_dim,
        sink_len,
    ...
    let group = n_heads as usize; // n_kv_heads == 1 in this test
    let window_base = sink_len + window_len; // one full block already evicted
    let cur_len_max = window_base + window_fill; // deepest row's cur_len
    let pos_base = cur_len_max - chunk_len;
    ...
    let q: Vec<f32> = synthetic(chunk_len as usize * n_heads as usize * hd, 1);
    let sink_k_f32 = synthetic(sink_len as usize * hd, 2);
    let sink_v_f32 = synthetic(sink_len as usize * hd, 3);
    let bulk_block_k = synthetic(window_len as usize * hd, 4);
    let bulk_block_v = synthetic(window_len as usize * hd, 5);
    let window_k_f32 = synthetic(window_len as usize * hd, 6);
    let window_v_f32 = synthetic(window_len as usize * hd, 7);
    ...
    let (k_codes, k_scales) = quantize_k(&bulk_block_k, window_len as usize, hd);
    let mut k_deq = vec![0f32; k_codes.len()];
    for t in 0..window_len as usize {
        for d in 0..hd {
            let idx = t * hd + d;
            k_deq[idx] = k_codes[idx] as f32 * k_scales[d];
        }
    }
    let (v_deq, v_scales_bulk, bulk_v_codes_bytes): (Vec<f32>, Vec<f32>, Vec<u8>) = if v_bits == 8 {
        let (codes, scales) = quantize_v_q8(&bulk_block_v, window_len as usize, hd);
        let mut out = vec![0f32; codes.len()];
        for t in 0..window_len as usize {
            for d in 0..hd {
                let idx = t * hd + d;
                out[idx] = codes[idx] as f32 * scales[t];
            }
        }
        (out, scales, codes.iter().map(|&c| c as u8).collect())
    } else {
        let (packed, scales) = quantize_v_q4(&bulk_block_v, window_len as usize, hd);
        let mut out = vec![0f32; window_len as usize * hd];
        for t in 0..window_len as usize {
            let scale = scales[t];
            for pair in 0..hd / 2 {
                let byte = packed[t * (hd / 2) + pair];
                ...
                out[t * hd + 2 * pair] = lo as f32 * scale;
                out[t * hd + 2 * pair + 1] = hi as f32 * scale;
            }
        }
        (out, scales, packed)
    };

    // Per-position K/V rows a correct kernel run must reproduce, for every
    // position up to the deepest row's cur_len.
    let mut k_by_pos: Vec<Vec<f32>> = Vec::with_capacity(cur_len_max as usize);
    let mut v_by_pos: Vec<Vec<f32>> = Vec::with_capacity(cur_len_max as usize);
    for pos in 0..cur_len_max as usize {
    ...
    let mut expected = vec![0f32; chunk_len as usize * n_heads as usize * hd];
    for i in 0..chunk_len as usize {
        let cur_len = pos_base as usize + i + 1;
        let q_row = &q[i * n_heads as usize * hd..(i + 1) * n_heads as usize * hd];
        let out_row = cpu_reference(q_row, &k_by_pos, &v_by_pos, cur_len, n_heads as usize, hd);
        expected[i * n_heads as usize * hd..(i + 1) * n_heads as usize * hd]
    ...
    let partial_len = chunk_len as usize * n_heads as usize * n_splits as usize;
    let buf_out = DeviceBuffer::<f32>::new(chunk_len as usize * n_heads as usize * hd).unwrap();
    let mut buf_partial_out = DeviceBuffer::<f32>::new(partial_len * hd).unwrap();
    let buf_partial_m = DeviceBuffer::<f32>::new(partial_len).unwrap();
    let buf_partial_l = DeviceBuffer::<f32>::new(partial_len).unwrap();
    // Poison partial scratch — an under-write should show up as garbage.
    buf_partial_out
    ...
    let split_len = cur_len_max.div_ceil(n_splits);
    ...
    let partial_cfg = LaunchConfig {
        ...
        block: (32, group_u32, BR),
        shared_mem_bytes: 2 * BC * head_dim * std::mem::size_of::<f32>() as u32,
    };
    ...
    let mut actual = vec![0.0f32; chunk_len as usize * n_heads as usize * hd];
    ...
    for (i, (got, want)) in actual.iter().zip(&expected).enumerate() {
        ...
        assert!(
            diff <= tol,
            "mixed flash prefill (v_bits={v_bits}) out[{i}]: got {got}, want {want} (diff {diff}, \
             tol {tol})"
        );
    }
}
...
#[test]
fn mixed_flash_q8_sink_bulk_and_window_multi_row() {
    run_mixed_flash(
        Scenario {
            n_heads: 4,
            head_dim: 64,
...
#[test]
fn mixed_flash_q4_sink_bulk_and_window_multi_row() {
    run_mixed_flash(
        Scenario {
            n_heads: 4,
            head_dim: 64,
...
#[test]
fn mixed_flash_q8_deep_multi_block_head_dim_256() {
    // head_dim 256, group 4 (Ornith's real full-attention GQA shape: 32
    // lanes * group(4) * BR(8) == 1024 threads/block, gfx1101's ceiling —
    // see `attn_prefill_flash.hip`'s module doc), a chunk spanning more
    // than one BR(8)-row tile.
    run_mixed_flash(
        Scenario {
            n_heads: 4,
            head_dim: 256,
...
#[test]
fn mixed_flash_q8_split_k_matches_single_split() {
    run_mixed_flash(
        Scenario {
            n_heads: 4,
            head_dim: 256,
...
#[test]
fn mixed_flash_q4_split_k_matches_single_split() {
    run_mixed_flash(
        Scenario {
            n_heads: 4,
            head_dim: 256,
