//! GPU integration tests for `gemv_q5_k`: fused dequant-GEMV that reads raw
//! GGUF Q5_K super-blocks directly from VRAM (dequantized in-register),
//! compared against rocml-core's proven-correct CPU dequant + a CPU dot
//! product. No real-tensor test here — see `gemv_q4_k.rs` for why.
mod common;

use common::{
    assert_close, expected_gemv, random_row_q5_k, run_gemv_kernel, Rng, K_BLOCK_ELEMS,
    Q5_K_BLOCK_BYTES,
};
...
fn run(m: u32, n: u32, seed: u32) {
    ...
    assert_close(&actual, &expected, "gemv_q5_k y");
}

#[test]
fn gemv_q5_k_non_power_of_two_m() {
    run(17, 768, 21);
}

#[test]
fn gemv_q5_k_degenerate_single_row() {
    run(1, 256, 22);
}

#[test]
fn gemv_q5_k_large_shape() {
    run(256, 4096, 23);
}
